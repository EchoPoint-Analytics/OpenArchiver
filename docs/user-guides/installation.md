...
This guide will walk you through setting up ProofArchive using Docker Compose. This is the recommended method for deploying the application.
...
First, clone the ProofArchive repository to your machine:
...
Before configuring the application, you **must** create a directory on your host machine where ProofArchive will store its data (such as emails and attachments). Manually creating this directory helps prevent potential permission issues.
...
This will configure the ProofArchive application to connect to your services instead of starting the default ones.
...
| `STORAGE_LOCAL_ROOT_PATH`      | The root path for ProofArchive app data.                                                                   | `/var/data/proofarchivsender` |
...
After successfully deploying and logging into ProofArchive, the next step is to configure your ingestion sources to start archiving emails.
...
To update your ProofArchive instance to the latest version, run the following commands:
...
If you are deploying ProofArchive on [Coolify](https://coolify.io/), it is recommended to let Coolify manage the Docker networks for you. This can help avoid potential routing conflicts and simplify your setup.
...
Look through the list for a volume name that ends with `_archiver-data`. The part before that will be your project's directory name. For example, if your project is in a folder named `ProofArchive`, the volume will be `ProofArchive_archiver-data` But it can be a randomly generated hash.
