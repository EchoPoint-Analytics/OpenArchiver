...
Meilisearch, the search engine used by ProofArchiveSender, requires a manual data migration process when upgrading to a new version. This is because Meilisearch databases are only compatible with the specific version that created them.

If an ProofArchiveSender upgrade includes a major Meilisearch version change, you will need to migrate your search index by following the process below.
...
### Step 2: Upgrade Your ProofArchiveSender Instance

Once the dump is successfully created, you can proceed with the standard ProofArchiveSender upgrade process.
